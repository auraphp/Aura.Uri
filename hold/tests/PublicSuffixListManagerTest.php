...
class PublicSuffixListManagerTest extends \PHPUnit_Framework_TestCase
{
    ...
    protected function setUp()
    {
        ...
        $this->httpAdapter = $this->getMock('\Aura\Uri\HttpAdapter\HttpAdapterInterface');
    ...
    {
        $content = file_get_contents(AURA_URI_TEST_ROOT . '/_files/' . PublicSuffixListManager::AURA_URI_PSL_TEXT_FILE);
        ...
        $this->assertFalse(file_exists($this->cacheDir . '/' . PublicSuffixListManager::AURA_URI_PSL_TEXT_FILE));
        $this->assertFalse(file_exists($this->cacheDir . '/' . PublicSuffixListManager::AURA_URI_PSL_PHP_FILE));

        $this->listManager->refreshPublicSuffixList();

        $this->assertFileExists($this->cacheDir . '/' . PublicSuffixListManager::AURA_URI_PSL_TEXT_FILE);
        $this->assertFileExists($this->cacheDir . '/' . PublicSuffixListManager::AURA_URI_PSL_PHP_FILE);
    }
    ...
    public function testGetHttpAdapterReturnsDefaultCurlAdapterIfAdapterNotSet()
    {
        ...
        $this->assertInstanceOf('\Aura\Uri\HttpAdapter\CurlHttpAdapter', $listManager->getHttpAdapter());
    }
    ...
    public function testWritePhpCache()
    {
        ...
        $array = $this->listManager->parseListToArray(
            AURA_URI_TEST_ROOT . '/_files/' . PublicSuffixListManager::AURA_URI_PSL_TEXT_FILE
        );
        $this->assertGreaterThanOrEqual(230000, $this->listManager->writePhpCache($array));
        $this->assertFileExists($this->cacheDir . '/' . PublicSuffixListManager::AURA_URI_PSL_PHP_FILE);
        $publicSuffixList = include $this->cacheDir . '/' . PublicSuffixListManager::AURA_URI_PSL_PHP_FILE;
    ...
    {
        copy(
            AURA_URI_TEST_ROOT . '/_files/' . PublicSuffixListManager::AURA_URI_PSL_PHP_FILE,
            $this->cacheDir . '/' . PublicSuffixListManager::AURA_URI_PSL_PHP_FILE
        );
        $this->assertFileExists($this->cacheDir . '/' . PublicSuffixListManager::AURA_URI_PSL_PHP_FILE);
        $publicSuffixList = $this->listManager->getList();
        $this->assertInstanceOf('\Aura\Uri\PublicSuffixList', $publicSuffixList);
        $this->assertGreaterThanOrEqual(300, count($publicSuffixList));
        $this->assertTrue(array_key_exists('stuff-4-sale', $publicSuffixList['org']) !== false);
        $this->assertTrue(array_key_exists('net', $publicSuffixList['ac']) !== false);
    }
